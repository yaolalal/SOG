...
class CosineSimCodebook(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            # print('🔥 🔥 🔥 EMA moving 🔥 🔥 🔥')
            # 💡 💡 💡 使用 EMA（Exponential Moving Average）更新 codebook 的 prototype 向量和 cluster size
...
class VectorQuantize(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            # ⚠️ 相当于 quantize = x + a（常数） -- 反向梯度通过x传
        ...
        if self.training:
            ...
            if exists(mask):
                ...
                codebook_loss = codebook_loss[mask].mean()
            else:
                commit_loss = F.mse_loss(detached_quantize, x)
                # 💡 添加codebook loss
                codebook_loss = F.mse_loss(quantize, x.detach())
            loss = loss + commit_loss * self.commitment_weight + codebook_loss * self.codebook_weight
            ...
            if self.orthogonal_reg_weight > 0:
                ...
                if self.orthogonal_reg_active_codes_only:
                    ...
                    if (unique_code_ids >= codebook.shape[1]).any() or (unique_code_ids < 0).any():
                        print(f"unique_code_ids error! max={unique_code_ids.max()}, min={unique_code_ids.min()}, codebook size={codebook.shape[0]}")
                        print('codebook shape:',codebook.shape)
                        assert False, "unique_code_ids out of range"
                    codebook = codebook[:, unique_code_ids, :]

                # num_codes = codebook.shape[1]
                unique_code_ids = torch.unique(embed_ind)
                num_codes = len(unique_code_ids)              
        ...
        elif self.orthogonal_reg_weight > 0:
            codebook = self._codebook.embed
            ...
            unique_code_ids = torch.unique(embed_ind)
            num_codes = len(unique_code_ids)
            codebook_loss = F.mse_loss(quantize.detach(), x.detach())
            commit_loss = codebook_loss
            orthogonal_reg_loss = orthogonal_loss_fn(codebook.detach())
            loss = loss + orthogonal_reg_loss * self.orthogonal_reg_weight + commit_loss * self.commitment_weight + codebook_loss * self.codebook_weight
        ...
        detailed_loss = {
                "vq_loss": loss,
                "orthogonal_reg_loss": orthogonal_reg_loss * self.orthogonal_reg_weight,
                "commit_loss": commit_loss * self.commitment_weight,
                "codebook_loss": codebook_loss * self.codebook_weight,
                "num_codes": num_codes,
            }

        return quantize, embed_ind, loss, dist, self._codebook.embed, detailed_loss
